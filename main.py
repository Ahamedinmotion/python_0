...
def test_ft_exercise(exercise_file_name):
    ...
    try:
        ...
        if exercise_file_name == "ft_hello_garden":
            module_path = f"ex0.{exercise_file_name}"
        elif exercise_file_name == "ft_plot_area":
            module_path = f"ex1.{exercise_file_name}"
        elif exercise_file_name == "ft_harvest_total":
            module_path = f"ex2.{exercise_file_name}"
        elif exercise_file_name == "ft_plant_age":
            module_path = f"ex3.{exercise_file_name}"
        elif exercise_file_name == "ft_water_reminder":
            module_path = f"ex4.{exercise_file_name}"
        elif exercise_file_name in [
            "ft_count_harvest_iterative",
            "ft_count_harvest_recursive",
        ]:
            module_path = f"ex5.{exercise_file_name}"
        elif exercise_file_name == "ft_garden_summary":
            module_path = f"ex6.{exercise_file_name}"
        elif exercise_file_name == "ft_seed_inventory":
            module_path = f"ex7.{exercise_file_name}"
        else:
            raise ImportError("Unknown exercise")

        # Import module dynamically
        ft_module = importlib.import_module(module_path)
